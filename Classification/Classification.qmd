...
Here, we will introduce the functions `confusion_matrix()` and `accuracy_score()` to evaluate the performance of a logistic regression classifier.
...
The quantity 
$$e^{\beta_0 + \beta_1 X_1 + \beta_2 X_2 + \cdots + \beta_p X_p} = \frac{P(Y = 1 | \boldsymbol{X} = \boldsymbol{x})}{1 - P(Y = 1 | \boldsymbol{X} = \boldsymbol{x})}$$ 
is known as the ["**odds**"]{style="color:gold;"} ratio.
...
Consider the banknote classification problem where $Y = 1$ implies counterfeit and $Y = 0$ genuine note. We have four predictors in $\boldsymbol{X} =  (X_1, X_2, X_3, X_4)$ summarizing specific characteristics of a banknote. 

</br>

The odds ratio is $e^{\beta_0 + \beta_1 X_1 + \beta_2 X_2 + \beta_3 X_3 + \beta_p X_4} = \frac{P(Y = 1 | \boldsymbol{X} = \boldsymbol{x})}{1 - P(Y = 1 | \boldsymbol{X} = \boldsymbol{x})}$
...
-   Strictly speaking, the test dataset must be named [**validation dataset**]{style="color:orange;"}.
