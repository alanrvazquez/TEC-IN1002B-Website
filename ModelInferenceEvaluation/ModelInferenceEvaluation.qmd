...
It was invented by [William Gosset](https://es.wikipedia.org/wiki/William_Sealy_Gosset) when he worked at the Guinness Brewery in Ireland.
...
The **p-value** is the probability that the test statistic $t_0$ will take on a value that is at least as extreme as the observed value of the statistic when the null hypothesis ($H_0$) is true.
...
Since the p-value is smaler than $\alpha = 0.05$, we reject $H_0$ for acceleration and weight. Therefore, **weight and acceleration have a significant association with the mpg of a car**.
...
-   It turns out that $R^2$ is the square of the correlation between the observed response and predicted values.
:::
