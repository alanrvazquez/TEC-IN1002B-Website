...
In python, the quantiles are calculated using the function `quantile()`.
...
First, we ensure that the variable `Type` is categorical.
...
Next, we create the frequency table using **pandas**.

```{python}
#| echo: true
#| output: true

table_counts_type = glass_data['Type'].value_counts()
print(table_counts_type)
```

## 

Finally, we create the bar chart using the function `countplot()` from **seaborn**.
