...
The categories can be represented by names, labels or even numbers. For a [nominal]{style="color:darkred;"} categorical variable, their use in regression requires **dummy variables**, which are numeric.
...
***Initially, a nominal categorical variable with*** $k$ ***categories requires*** $k$ ***dummy variables.***
...
We will use a validation dataset with 20% of the observations in `auto_data`. The other 80% will be in the training dataset.
...
## Coding ordinal categorical variables

</br>

An [ordinal]{style="color:darkblue;"} categorical variable has categories with a natural order.
...
One way to turn an ordinal categorical variable into a series of numeric ones is using [**polynomial contrasts**]{style="color:green;"}.

## Polynomial contrasts

Polynomial contrasts encode the **order and trend** of an ordinal variable.

They can capture:

-   A *linear* trend: the linear change in the response when we increase the level of the ordinal variable.

-   A *quadratic* trend: the quadratic change in the response when we increase the level of the ordinal variable.

-   A *cubic* trend: a nonlinear relationship between the response and the ordinal variable.
...
df = pd.DataFrame(columns=['Level', 'Linear', 'Quadratic', 'Cubic'])
df['Level'] = ['Poor', 'Fair', 'Good', 'Excellent']
df['Linear'] = [65, 70, 75, 80]
df['Quadratic'] = [65, 80, 80, 65]
df['Cubic'] = [65, 75, 70, 75]

# Plot
plt.figure(figsize=(8, 5))
for col in ['Linear', 'Quadratic', 'Cubic']:
    plt.plot(df['Level'], df[col], marker='o', label=col)

plt.title('Polynomial Contrasts for an Ordinal Variable with 4 Levels', fontsize = 12)
plt.xlabel('Material Quality', fontsize = 14)
...
The variable `CHAS` is a nominal categorical variable because it categories ("No" and "Yes") denote whether the house is next to the Charles river.

Therefore, we turn the predictor into categorical using `.Categorical()` function from **pandas**.
...
The variable `RAD` is an ordinal categorical variable because it is the index of accessibility to radial highways. The categories of thei variable are `Low`, `Medium`, and `High`.

</br>

To turn the predictor into categorical, we use `.Categorical()` function from **pandas**. However, we must use two extra inputs to ensure that Python knows the right order of the categories. These inputs are `categories` and `ordered`.
...
For example, consider the distribution of the predictor `DIS` (weighted mean of distances to five Boston employment centers) in `Boston_training`.
...
DIS_transform = power_transform(Boston_training[['DIS']], method = "box-cox")
