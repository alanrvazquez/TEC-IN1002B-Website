...
2.  Standardizing the units of the predictors
...
The categories can be represented by names, labels or even numbers. For a nominal categorical variable, their use in regression requires **dummy variables**, which are numeric.
...
An ordinal categorical variable has categories with a natural order.
...
- A *quadratic* trend: a nonlinear relationship between the response and the ordinal variable.
...
The variable `CHAS` is a nominal categorical variable because it categories ("0" and "1") denote whether the house is next to the Charles river.
