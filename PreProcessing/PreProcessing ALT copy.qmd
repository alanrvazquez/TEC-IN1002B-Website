...
4.  Standarization
5.  Transforming categorical predictors
6.  Filtering
...
We will not use all the functions from the **scikit-learn** library. Instead, we will use specific functions from the sub-libraries **model_selection**, **preprocessing**, **feature_selection**, and **impute**.
...
::: incremental
-   [**Training data**]{style="color:blue;"} is data that we use to train or construct the estimated model $\hat{f}(\boldsymbol{X})$.
...
We consider a data set related to Glass identification. The data has 214 glass samples labeled as one of seven glass categories. There are nine predictors including the refractive index (RI) and percentages of eight elements: Na, Mg, Al, Si, K, Ca, Ba, and Fe. The response is the type of glass.
...
X_full = glass_data.drop(columns = ['Type'])

# Set full matrix of responses.
Y_full = glass_data['Type']
...
We assume that the response under study is `species` and the other the variables are predictors.

## Create training dataset

We create the training dataset called `penguins_train`. We will apply the 
...
# Set full matrix of predictors.
X_full = penguins_data.drop(columns = ['species'])

# Set full matrix of responses.
Y_full = penguins_data['species']

# Split the dataset.
penguins_train, p_val, Y_train, Y_val = train_test_split(X_full, Y_full, 
                test_size=0.3)
```
...
[***In a nutshell, some predictors will have a higher impact in the model due to its unit and not its information provided to it.***]{style="color:pink;"}
...
complete_sbAuto = sb_Auto.dropna()
```
