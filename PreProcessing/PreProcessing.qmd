...
3.  Standarization
4.  Transforming categorical predictors
5.  Filtering
...
For example, let's imput the missing values of all categorical variables using this strategy.
...
penguins_data = cat_imputer.fit_transform(penguins_data)
```
...
Many of the predictive models we will see later have issues with [**numeric predictors**]{style="color:blue;"} with different units:
...
plt.scatter(x = 1.82, y = 2.53, color = 'red')
plt.scatter(x = -0.91, y = -1.60, color = 'red')
...
reduced_auto = complete_sbAuto['acceleration']
```
